...
class UserDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['events'] = Event.objects.filter(
            users_watching=self.get_object())
        context['parties'] = ViewingParty.objects.filter(
            attendees=self.get_object())
...
class EventDetail(DetailView):
    model = Event
    fields = ['title', 'location', 'event_type', 'start_date', 'start_time', 'end_date',
              'end_time', 'best_date', 'best_time', 'description', 'users_watching', 'created_by']
    ...
    def get_context_data(self, **kwargs):
        ...
        for location in party_locations:
            geo_request = requests.get(
                'https://api.mapbox.com/geocoding/v5/mapbox.places/<str:location>.json')
...
class PartyCreate(LoginRequiredMixin, CreateView):
    model = ViewingParty
    fields = ['name', 'party_location', 'start_date', 'start_time',
              'end_date', 'end_time', 'description', 'event']
...
class PartyUpdate(LoginRequiredMixin, UpdateView):
    model = ViewingParty
    fields = ['name', 'party_location', 'start_date',
